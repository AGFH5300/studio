...
export async function POST(request:Request){
  try{
    ...
    const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.LEAD_FROM_EMAIL||"STUDIO/AE <onboarding@resend.dev>",to:[to],reply_to:email,subject:`New website enquiry — ${name}`,text:lines.join("\n"),html:`<h2>New website enquiry</h2><pre style="font:14px/1.6 Arial;white-space:pre-wrap">${escape(lines.join("\n"))}</pre>`})});
