...
export default function ContactForm(){
  ...
  useEffect(()=>{const frame=requestAnimationFrame(()=>{try{const saved=sessionStorage.getItem("studioConfig");if(saved)setConfig(JSON.parse(saved))}catch{}});return()=>cancelAnimationFrame(frame)},[]);
  ...
  if(status==="sent")return <div className="success-state" role="status"><span>✓</span><small>ENQUIRY SENT</small><h3>Project received.</h3><p>Thank you. We’ll review the requirements and come back with a clear next step.</p><Link href="/">Return home ↗</Link></div>;
